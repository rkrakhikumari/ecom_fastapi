...
def add_to_cart(item: AddToCart, db: db_dependency, user_data: dict = Depends(get_customer)):
    ...
    if not product:
        raise HTTPException(status_code=404, detail='product not foud')

    if item.quantity > product.stock:
        raise HTTPException(
            status_code=400,
            detail=f"cannot add {item.quantity} units only {product.stock} unit in stock"
        )
    ...
    if existing_cart_item:
        ...
        if total_quantity > product.stock:
            raise HTTPException(status_code=400,detail=f"total quantity in cart would exceed stock. Available: {product.stock}")
        existing_cart_item.quantity += item.quantity
        db.commit()
        return {'msg': 'updated cart', 'product_name': product.name}

    cart_item = CartItem(user_id=user_data['user'].id,product_id=item.product_id,quantity=item.quantity
    )
